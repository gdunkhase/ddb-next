      ...
      <div class="row-fluid">
          ...
          <select class="facet facet-js" id="facet-js-${group}-${row}" name="facet-${group}-${row}" style="display:none" disabled="disabled">
            <g:each in="${facetSearchfields}">
              <g:if test="${it.searchType?.equals(enumSearchType)}">
                <option value="${it.name}" data-inputid="${it.name}${facetNameSuffix}-${group}-${row}"><g:message code="${languageTagPrefix}${it.name}"/></option>
              </g:if>
              <g:else>
                <option value="${it.name}" data-inputid="value-${group}-${row}"><g:message code="${languageTagPrefix}${it.name}"/></option>
              </g:else>
            </g:each>
          </select>
          ...
          <select class="facet facet-simple" name="facet-${group}-${row}">
            <g:each in="${facetSearchfields}">
              <g:if test="${it.searchType?.equals(textSearchType)}">
                <option value="${it.name}"><g:message code="${languageTagPrefix}${it.name}"/></option>
              </g:if>
            </g:each>
          </select>
        </div>
        ...
        <div class="span6">
          <g:each var="facetSearchfield" in="${facetSearchfields}">
            <g:if test="${facetSearchfield.searchType?.equals(enumSearchType)}">
              <select class="facet-values" id="${facetSearchfield.name}${facetNameSuffix}-${group}-${row}" name="${facetSearchfield.name}${facetNameSuffix}-${group}-${row}" style="display: none">
                <option value=""><g:message code="ddbnext.AdvancedSearch_PleaseSelect"/></option>
                <g:set var="key" value="${facetSearchfield.name}${facetNameSuffix}"/>
                <g:each var="facetValue" in="${facetValuesMap[key]}">
                  <option value="${facetValue.key}">${facetValue.value}</option>
                </g:each>
              </select>
            </g:if>
          </g:each>
        ...
        <div class="span3">
          <select class="match" name="match-${group}-${row}">
            <option value="ANY"><g:message code="ddbnext.AdvancedSearch_Any"/></option>
            <option value="ALL"><g:message code="ddbnext.AdvancedSearch_All"/></option>
            <option value="EXACT"><g:message code="ddbnext.AdvancedSearch_Exact"/></option>
          </select>
        </div>
      </div>
    </div>
