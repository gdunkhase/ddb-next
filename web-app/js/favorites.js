...
// var paramsArray = new Array(new Array('rows', this.value), new
// Array('offset', 0));
...
// var paramsArray = new Array(new Array('sort', this.value), new
// Array('offset', 0));
// fetchResultsList(addParamToCurrentUrl(paramsArray));
// $('.clear-filters').attr('href',
// $('.clear-filters').attr('href').replace(/sort=(RELEVANCE|ALPHA_DESC|ALPHA_ASC)/i,'sort='+this.value));
// return false;
// });
...
function addParamToUrl(arrayParamVal, path, urlString) {
  ...
  var queryParameters = {}, queryString = (urlString == null) ? currentUrl
      : urlString, re = /([^&=]+)=([^&]*)/g, m;
  while (m = re.exec(queryString)) {
    ...
    queryParameters[decodeURIComponent(m[1].replace(/\+/g, '%20'))]
        .push(decodeURIComponent(m[2].replace(/\+/g, '%20')));
  }
...
function removeParamFromUrl(arrayParamVal, path, urlString) {
  ...
  while (m = re.exec(queryString)) {
    ...
    queryParameters[keyParam].push(decodeURIComponent(m[2]
        .replace(/\+/g, '%20')));
  }
  $.each(arrayParamVal, function(key, value) {
    if (queryParameters[value[0]]
        && (paramIndex = $.inArray(value[1], queryParameters[value[0]])) > -1) {
      queryParameters[value[0]] = jQuery.grep(queryParameters[value[0]],
          function(cValue) {
